...
export class ChatEmpleados implements OnInit, AfterViewChecked {
  ...
  loadConversations(): void {
    ...
    this.convService.getConversationsForUser(currentUid).subscribe(list => {
      this.conversations = list
        .filter(conv => conv.participants?.includes(currentUid))
        .map(conv => {
          const otherUid = conv.participants?.find((p: string) => p !== currentUid);
          const otherUser = this.users.find((u: any) => u.uid === otherUid);
