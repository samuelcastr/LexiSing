...
export class SordomudoReportes implements OnInit, AfterViewInit {
  ...
  crearGraficoUsuarios(users: any[]): void {
    if (!document.getElementById('grafico-usuarios-sord')) return;
    ...
    this.Plotly.newPlot('grafico-usuarios-sord', [trace1, trace2], layout, { responsive: true });
  }

  crearGraficoConversaciones(conversations: any[]): void {
    if (!document.getElementById('grafico-conversaciones-sord')) return;
    ...
    this.Plotly.newPlot('grafico-conversaciones-sord', [trace], layout, { responsive: true });
  }

  crearGrafico3DActividad(conversations: any[]): void {
    if (!document.getElementById('grafico-3d-sord')) return;
    ...
    this.Plotly.newPlot('grafico-3d-sord', [trace], layout, { responsive: true });
  }

  crearGraficoMensajes(messages: any[]): void {
    if (!document.getElementById('grafico-mensajes-sord')) return;
    ...
    this.Plotly.newPlot('grafico-mensajes-sord', [trace], layout, { responsive: true });
  }

  goBack(): void {
    this.router.navigate(['/roles/sordomudo']);
  }
}
