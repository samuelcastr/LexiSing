...
class TextFormalizeView(APIView):
    ...
    def post(self, request):
        try:
            if not request.auth: return Response({'detail':'Autenticación requerida.'}, status=status.HTTP_401_UNAUTHORIZED)
            ...
            return Response({'texto_formal': resultado['texto'], 'gestos_originales': gestos, 'fuente': resultado['fuente']}, status=status.HTTP_200_OK)
        except Exception as e: return Response({'texto_formal':'','gestos_originales':[],'fuente':'fallback','error':str(e)}, status=status.HTTP_200_OK)
